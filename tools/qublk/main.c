...
#define QUBLK_DEFAULT_DEPTH 64
#define QUBLK_DEFAULT_NR_QUEUES 1
...
sub_run(struct xnvme_cli *cli)
{
	struct qublk_dev dev = {
		...
		.nr_queues = QUBLK_DEFAULT_NR_QUEUES,
		.depth = QUBLK_DEFAULT_DEPTH,
	...
	if (cli->given[XNVME_CLI_OPT_QDEPTH]) {
		dev.depth = (uint16_t)cli->args.qdepth;
	}
	if (cli->given[XNVME_CLI_OPT_NQUEUES]) {
		dev.nr_queues = (uint16_t)cli->args.nqueues;
	}
	...
	if (!xnvme_is_pow2(dev.depth) || dev.depth > QUBLK_MAX_QUEUE_DEPTH) {
	...
	if (!dev.xdev) {
		xnvme_cli_perr("Failed: xnvme_dev_open()", -errno);
		return -errno;
	}
